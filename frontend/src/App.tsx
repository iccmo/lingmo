...
// Lazy-loaded routes
const NovelOverview = lazy(() => import('src/pages/NovelOverview').then(m => ({ default: m.NovelOverview })));
...
const NovelAnalysis = lazy(() => import('src/pages/NovelAnalysis').then(m => ({ default: m.NovelAnalysis })));
const NovelCharacters = lazy(() => import('src/pages/NovelCharacters').then(m => ({ default: m.NovelCharacters })));
const NovelPublish = lazy(() => import('src/pages/NovelPublish').then(m => ({ default: m.NovelPublish })));
const NovelTools = lazy(() => import('src/pages/NovelTools').then(m => ({ default: m.NovelTools })));
...
function Footer() {
 ...
 <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-success-soft0" />
 </>
 ) : (
...
function AppLayout({ mode, setMode, dark, toggleDark, sidebarOpen, toggleSidebar }: {
 ...
 {/* Novel routes */}
 <Route path="/novels/:id" element={<NovelOverview />} />
 <Route path="/novels/:id/write" element={<WriterView />} />
 <Route path="/novels/:id/analysis" element={<NovelAnalysis />} />
 <Route path="/novels/:id/characters" element={<NovelCharacters />} />
 <Route path="/novels/:id/world" element={<WorldEditor />} />
 <Route path="/novels/:id/publish" element={<NovelPublish />} />
 <Route path="/novels/:id/tools" element={<NovelTools />} />
