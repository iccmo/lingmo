...
import { ClipboardList, Dna, Lightbulb, Smartphone, Target } from 'lucide-react';
...
function personaReaction(summary: string, persona: typeof READER_PERSONAS[number]): string[] {
 ...
 if (/突然|莫名|不知|为什么/.test(s)) reactions.push('这里有逻辑漏洞，为什么会这样？前面没解释');
 }
...
function paragraphReactions(paragraphs: string[]): { text: string; reaction: string; emoji: string; color: string }[] {
 ...
 if (/战斗|打斗|激战|对决|杀|砍|刺|爆/.test(p)) { reaction = '肾上腺素飙升'; emoji = ''; color = 'bg-destructive-soft dark:bg-red-900/30 border-destructive/20 '; }
 ...
 else if (/笑|搞笑|逗|吐槽|欢乐|囧/.test(p)) { reaction = '笑出声'; emoji = '😂'; color = 'bg-warn-soft dark:bg-amber-900/30 border-warn/20 '; }
 else if (/泪|哭|悲伤|死|失去|离别/.test(p)) { reaction = '眼眶湿润'; emoji = '😢'; color = 'bg-info-soft dark:bg-blue-900/30 border-info/20 dark:border-blue-800'; }
 else if (/突破|升级|成功|赢了|击败|获得/.test(p)) { reaction = '极度舒适'; emoji = '😌'; color = 'bg-success-soft dark:bg-emerald-900/30 border-success/20 '; }
...
export function CreativeLab({ chapters, genre: _genre, novelId }: { chapters?: ChapterMeta[]; genre: string; novelId: string }) {
 ...
 <Lightbulb size={12} className="inline" /> 切换不同读者角色，看不同人群怎么评价你的章节
 </p>
 </div>
 )}
 ...
 {/* Tropes detected */}
 <div>
 <h4 className="text-xs font-semibold text-ink mb-2"><ClipboardList size={12} className="inline" /> 检测到的套路</h4>
 {tropes.length === 0 ? (
 ...
 <h4 className="text-xs font-semibold text-ink mb-2"><Dna size={12} className="inline" /> 你的作者 DNA</h4>
 <div className="grid grid-cols-2 gap-3">
 <div className="p-3 rounded-lg bg-success-soft/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30">
 <p className="text-[10px] text-success font-medium mb-1">💪 强项</p>
 {dna.strengths.map(s => (
 <p key={s} className="text-xs text-ink">• {s}</p>
 ))}
 </div>
 <div className="p-3 rounded-lg bg-warn-soft/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30">
 <p className="text-[10px] text-warn font-medium mb-1"><Target size={12} className="inline" /> 待提升</p>
 ...
 <Lightbulb size={12} className="inline" /> 推荐题材：<span className="font-semibold">{dna.bestGenre}</span>
 </p>
 </div>
 </div>
 )}
 ...
 <div className="p-4 rounded-lg bg-success-soft/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30">
 <p className="text-xs font-semibold text-success mb-1">最强章</p>
 ...
 <p className="text-xs font-semibold text-accent mb-1"><ClipboardList size={12} className="inline" /> 发布策略</p>
 <p className="text-sm text-ink leading-relaxed">{advice.strategy}</p>
 </div>

 {/* Platform tips */}
 <div className="pt-3 border-t border-border">
 <h4 className="text-xs font-semibold text-ink mb-2"><Smartphone size={12} className="inline" /> 平台适配建议</h4>
 <div className="space-y-2 text-[11px]">
 <div className="flex gap-2">
 <span className="text-destructive font-medium shrink-0">番茄小说:</span>
 <span className="text-ink-muted">章节2000-4000字最佳，结尾必须有强钩子，标题要吸睛。前3章决定80%留存率。</span>
 </div>
 <div className="flex gap-2">
 <span className="text-info font-medium shrink-0">起点中文网:</span>
 <span className="text-ink-muted">章节3000-6000字，允许慢热但要有深度。读者更看重世界观和人物弧线。</span>
 </div>
 <div className="flex gap-2">
 <span className="text-success font-medium shrink-0">纵横中文网:</span>
