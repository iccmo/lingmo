...
function levelStyle(level: PhraseEntry['level']): {
 ...
 if (level === 'overused') {
 return {
 bg: 'bg-destructive-soft dark:bg-red-950/20',
 ...
 if (level === 'cliche') {
 return {
 bg: 'bg-warn-soft dark:bg-amber-950/20',
 text: 'text-warn ',
...
export function WordFrequency({ content, label }: Props) {
 ...
 <div className="p-2.5 rounded-lg bg-destructive-soft/50 dark:bg-red-950/20 border border-destructive/20 text-[11px] text-destructive ">
 ...
 ? 'bg-destructive-soft0/10 dark:bg-destructive-soft0/15'
 : entry.level === 'cliche'
 ? 'bg-warn-soft0/10 dark:bg-warn-soft0/15'
 : 'bg-accent/5'
 }`}
