...
const GRADE_COLORS: Record<string, string> = {
 ...
 A: 'bg-success-soft text-emerald-800 border-success/20 dark:bg-emerald-900/40 dark:text-emerald-300 ',
 B: 'bg-info-soft text-sky-800 border-info/20 dark:bg-sky-900/40 dark:text-sky-300 ',
 C: 'bg-warn-soft text-amber-800 border-warn/20 dark:bg-amber-900/40 dark:text-amber-300 ',
 D: 'bg-destructive-soft text-red-800 border-destructive/20 dark:bg-red-900/40 dark:text-red-300 ',
};
...
export function ChapterList({ chapters, novelId, onDelete, onRegenerate }: Props) {
 ...
 if (ratio > 10) return { pct: 80, label: '对话', color: 'text-info' };
 if (ratio > 4) return { pct: 50, label: '叙述', color: 'text-ink-muted' };
 return { pct: 25, label: '描写', color: 'text-warn' };
 }
 ...
 attentionFilter ? 'bg-warn-soft text-warn border-warn/20 dark:bg-amber-950/20 ' : 'border-border text-ink-muted hover:text-ink'
 }`}>
 {attentionFilter ? <><Search size={12} className='mr-1' /> 只看需关注</> : <><Search size={12} className='mr-1' /> 需关注</>}
 </button>
 ...
 className="text-[10px] px-2 py-1 rounded border border-border text-ink-muted hover:text-ink transition-colors">
 <BookOpen size={12} className='mr-1' /> 下一未读 (Ch{firstUnread.number})
 </button>
 );
 } catch { return null; }
 })()}
 ...
 const ctxItems = [
 { icon: 'PenLine', label: '编辑', onClick: () => {
 toggleChapter(ch.number);
 setTimeout(() => enterEditMode(), 500);
 }},
 { icon: 'RefreshCw', label: '重写本章', onClick: () => {
 if (onRegenerate) {
 ...
 { icon: 'Pin', label: pinned.has(ch.number) ? '取消置顶' : '置顶', onClick: () => togglePin(ch.number) },
 { icon: 'Tag', label: '标签', onClick: () => openTagPicker(ch.number) },
 { icon: 'CheckCircle2', label: approvals[ch.number] === 'approved' ? '已审' : approvals[ch.number] === 'revise' ? '待改→草稿' : '草稿→已审', onClick: () => cycleApproval(ch.number) },
 ...(onDelete ? [{
 icon: 'Trash2', label: '删除本章', danger: true as const,
 ...
 pinned.has(ch.number) ? 'text-warn' : 'text-ink-subtle opacity-0 group-hover:opacity-100'
 }`}
 ...
 {styleRefs.has(ch.number) ? <Star size={12} className='text-warn' fill='currentColor' /> : <Star size={12} className='text-ink-subtle' />}
 </button>
 )}
 {/* Approval toggle */}
 <button onClick={e => cycleApproval(ch.number, e)}
 className={`text-[10px] shrink-0 transition-all ${
 approvals[ch.number] === 'approved' ? 'text-success'
 : approvals[ch.number] === 'revise' ? 'text-warn'
 ...
 {approvals[ch.number] === 'approved' ? <CheckCircle2 size={12} className='text-success' /> : approvals[ch.number] === 'revise' ? <FileText size={12} className='text-warn' /> : <span className='w-3 h-3 rounded-full border border-border inline-block' />}
 </button>
 {/* Read status dot */}
 {(() => {
 try {
 ...
 <span className="text-[10px] px-1.5 py-0.5 rounded bg-success-soft text-success border border-success/20 dark:bg-emerald-900/30 font-semibold shrink-0"> 已发布</span>
 ) : (
 ...
 className="text-[10px] text-success hover:text-success font-medium">
 {saving ? '保存中...' : '💾 保存'}
 </button>
 <span className="text-[10px] text-ink-subtle">|</span>
 ...
 className="text-[10px] text-ink-subtle hover:text-accent">
 <PenLine size={12} className='mr-1' /> 编辑
 </button>
 )}
 <span className="text-[10px] text-ink-subtle">|</span>
 {content && (
 ...
 <Film size={12} className='mr-1' /> 场景
 </button>
 )}
 ...
 disabled={analyzing}
 className="text-[10px] text-ink-subtle hover:text-accent disabled:opacity-50">
 {analyzing ? <BarChart3 size={12} className='mr-1 animate-spin' /> : <TrendingUp size={12} className='mr-1' />} 分析
 </button>
 )}
 <span className="text-[10px] text-ink-subtle">|</span>
 ...
 <Scissors size={12} className='mr-1' /> 克制编辑结果
 </span>
 <button onClick={() => {
 ...
 <p className="text-[11px] text-ink-muted"><FileText size={11} className='mr-1' /> 私人笔记</p>
 ...
 {[
 { key: '对话', icon: 'MessageSquare', label: '对话自然度' },
 { key: '节奏', icon: '🏃', label: '节奏感' },
 { key: '钩子', icon: 'Anchor', label: '结尾钩子' },
 { key: '描写', icon: 'Palette', label: '场景描写' },
 { key: '人物', icon: 'Users', label: '人物塑造' },
 { key: '爽感', icon: 'Flame', label: '爽感/高潮' },
 ].map(f => (
 ...
 className="text-xs px-2.5 py-1 rounded-md bg-accent-soft text-accent hover:bg-accent-soft/80 transition-colors">
 <Search size={12} className='mr-1' /> 事实核查
 </button>
