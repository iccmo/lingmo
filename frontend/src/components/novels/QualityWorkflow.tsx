...
export function QualityWorkflow({ novelId, lastQuality, onNavigate }: {
 ...
 ▸ 质量工作流
 <span className="text-success">{doneCount}/{totalSteps}</span>
 {warnCount > 0 && <span className="text-warn">({warnCount}待优化)</span>}
 </button>
 </div>
 );
 }
 ...
 <span className="text-base"><Target size={12} className="inline" /></span>
 <h3 className="font-heading text-sm font-semibold text-ink">质量工作流</h3>
 <span className={`text-[10px] font-medium ${doneCount === totalSteps ? 'text-success' : doneCount >= 3 ? 'text-warn' : 'text-ink-subtle'}`}>
 ...
 status === 'warn' ? 'text-warn ' : 'text-ink-muted'
 }`}>
 {/* Status icon */}
 <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${
 status === 'done' ? 'bg-success-soft text-success dark:bg-emerald-900/30 ' :
 status === 'warn' ? 'bg-warn-soft text-warn dark:bg-amber-900/30 ' :
 ...
 status === 'warn' ? 'text-warn' : 'text-ink-subtle'
 }`}>{detail}</span>
 {/* Connector line */}
 ...
 <p className="text-[10px] text-success text-center mt-2"><CheckCircle2 size={12} className="text-success inline" /> 所有准备完成——可以开始生成高质量章节了</p>
 ) : (
