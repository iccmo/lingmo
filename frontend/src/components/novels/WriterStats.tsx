...
export function WriterStats({ novelId, totalChapters, totalWords, chapters }: {
 ...
 if (change > 10) return { emoji: '🚀', label: '加速', color: 'text-success' };
 if (change < -10) return { emoji: '🐢', label: '放缓', color: 'text-warn' };
 return { emoji: '', label: '稳定', color: 'text-ink-muted' };
 }, [velocityData]);
 ...
 <span className={pct >= 100 ? 'text-success font-semibold' : ''}>
 {pct >= 100 ? '🎉 ' : ''}{pct}%
 </span>
 ...
 {pct >= 100 && (
 <p className="text-[11px] text-success font-medium text-center animate-[fadeSlideIn_0.3s_ease-out]">
 恭喜！已完成目标字数！
 </p>
 )}
 </div>
 ...
 <Target size={12} className="inline" /> 预计 {estimatedCompletion} 完本
 </span>
 )}
 </div>
