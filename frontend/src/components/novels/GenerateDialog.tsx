...
const BASE_SUGGESTIONS = [
 ...
 { emoji: '', label: '世界观展开', text: '揭示更多世界观设定和背景' },
];
...
const SOUL_SUGGESTIONS: Record<string, { emoji: string; label: string; text: string }[]> = {
 ...
 { emoji: '', label: '景物暗示', text: '用天气/环境的变化暗示角色的内心——雨天的沉默比晴天的告白更有力' },
 ],
 'body-mind': [
 ...
 'desire-constraint': [
 { emoji: '', label: '两难选择', text: '角色面对两个都不想选的选项——但必须选一个。写出代价。' },
...
export function GenerateDialog({ open, onClose, onGenerate, chapterNumber, prevHook, novelId, prefillDirection }: {
 ...
 if (allReady) {
 return <div className="mb-3 text-[10px] text-success flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> 全部配置就绪</div>;
 }
 return (
 <div className="mb-3 p-2 rounded-lg text-[10px] bg-paper border border-border">
 <div className="flex items-center gap-2 flex-wrap">
 {status.map(s => (
 <label key={s.key} className={`flex items-center gap-1 cursor-pointer ${s.ready ? 'text-success ' : 'text-ink-subtle hover:text-ink'}`}
 onClick={s.autoFill}>
 ...
 <Zap size={12} className="inline" /> 全部补全
 </button>
 ...
 <p className="text-[10px] text-ink-muted mb-1"><Brain size={12} className="inline" /> 记忆注入（让 AI 写出只有你能写的东西）</p>
 <input
 value={memoryInput}
 ...
 qualityThreshold >= 0.8 ? 'text-destructive' : qualityThreshold >= 0.65 ? 'text-warn' : 'text-success'
 }`}>
