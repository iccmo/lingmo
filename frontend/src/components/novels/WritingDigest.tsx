...
import { AlertTriangle, Brain, ClipboardList, Lightbulb } from 'lucide-react';
...
export function WritingDigest({ chapters, novelId }: { chapters?: ChapterMeta[]; novelId?: string }) {
 ...
 <h3 className="font-heading text-base font-semibold text-ink mb-3"><ClipboardList size={12} className="inline" /> 写作日报</h3>
 ...
 {digest.chaptersToday === 0 && <span className="text-warn">今天还没写哦</span>}
 </div>

 {/* Best & weakest */}
 {digest.bestChapter.num > 0 && (
 <div className="flex gap-4 text-[11px] mb-3">
 <span className="text-success">最佳: Ch{digest.bestChapter.num} ({digest.bestChapter.score.toFixed(2)})</span>
 {digest.weakestChapter.num > 0 && (
 <span className="text-warn"><AlertTriangle size={12} className="text-warn inline" /> 最弱: Ch{digest.weakestChapter.num} ({digest.weakestChapter.score.toFixed(2)})</span>
 )}
 </div>
 )}
 ...
 <span className={qualityTrends.trend > 0.03 ? 'text-success' : qualityTrends.trend < -0.03 ? 'text-warn' : 'text-ink-subtle'}>
 {qualityTrends.trend > 0.03 ? '' : qualityTrends.trend < -0.03 ? '' : '→'}
 </span>
 ...
 <Lightbulb size={12} className="inline" /> 建议重点提升：
 {qualityTrends.weakDims.map((d, i) => (
 <span key={d.dim} className="text-warn font-medium">{d.dim}({d.avg.toFixed(1)}){i < qualityTrends.weakDims.length - 1 ? '、' : ''}</span>
 ))}
 </p>
 )}
 </div>
 ...
 <p className="text-[11px] font-medium text-ink mb-2"><Brain size={12} className="inline" /> 读者可能困惑</p>
