...
const GATE_STATUS_MAP: Record<string, { icon: string; label: string; color: string }> = {
  good: { icon: '✅', label: '良好', color: 'text-emerald-500' },
  warning: { icon: '⚠️', label: '注意', color: 'text-amber-500' },
  error: { icon: '🔴', label: '需修复', color: 'text-red-500' },
};
          ...
          <div className="space-y-2">
            ...
            {genStatus.overall !== undefined && (
              <div className="flex items-center gap-2">
                ...
                {genStatus.grade && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded border border-amber-200 bg-amber-50 text-amber-600 font-medium">
                    {genStatus.grade}
                  </span>
                )}
              </div>
