...
function paceLabel(score: number): { label: string; color: string } {
 if (score < 30) return { label: '慢节奏', color: 'text-info ' };
 if (score > 70) return { label: '快节奏', color: 'text-destructive ' };
 return { label: '适中', color: 'text-success ' };
}
...
export function PacingCurve({ chapters, contentMap }: Props) {
 ...
 {p.score}
 {p.abruptChange && (
 <span className="ml-1 text-warn" title="节奏突变">
 !
 </span>
 )}
 </td>
