...
import { Lightbulb, Pin, Search, Sparkles, Trash2 } from 'lucide-react';
...
export function SoulWorkshop({ novelId, chapters }: { novelId: string; chapters?: any[] }) {
 ...
 className="text-xs text-destructive hover:text-destructive dark:hover:text-destructive px-1">×</button>
 </div>
 ))}
 ...
 <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-destructive-soft text-destructive border border-destructive/20 dark:bg-red-900/20 flex items-center gap-1">
 {w}
 ...
 <p className="text-ink font-medium mb-1"><Pin size={12} className="inline" /> 灵魂陈述</p>
 ...
 <span className={`text-sm font-bold ${m.value >= 60 ? 'text-success' : m.value >= 35 ? 'text-warn' : 'text-destructive'}`}>
 {m.value}
 </span>
 </div>
 ...
 <p className="text-[10px] font-medium text-warn mb-1"><Search size={12} className="inline" /> 泛泛描写（建议替换为具体细节）</p>
 {checkResult.genericLines.map((l, i) => (
 <p key={i} className="text-[10px] text-amber-700 dark:text-warn leading-relaxed">「{l}」</p>
 ))}
 </div>
 )}

 {checkResult.explainedLines.length > 0 && (
 <div className="p-3 rounded-lg bg-destructive-soft/50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30">
 <p className="text-[10px] font-medium text-destructive mb-1"><Trash2 size={12} className="inline" /> 过度解释（删掉这些，读者会感谢你）</p>
 {checkResult.explainedLines.map((l, i) => (
 <p key={i} className="text-[10px] text-red-700 dark:text-destructive leading-relaxed">「{l}」</p>
 ))}
 </div>
 )}
 ...
 <p className="text-[10px] font-medium text-success mb-1"><Sparkles size={12} className="inline" /> 好细节（这就是灵魂）</p>
 {checkResult.detailLines.map((l, i) => (
 <p key={i} className="text-[10px] text-success dark:text-success leading-relaxed">「{l}」</p>
 ))}
 </div>
 )}
 ...
 <p key={i} className="text-[10px] text-accent"><Lightbulb size={12} className="inline" /> {s}</p>
 ))}
 </div>
 )}
 </div>
