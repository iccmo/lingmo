...
export function NovelArchitect({ novelId, chapters, totalChapters }: {
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink">长篇架构</h3>
 ...
 <Zap size={12} className="inline" /> {b.chapter - currentChapter}章后：{beatEmojis[b.type]} {b.event.slice(0, 20)}
 </span>
 ))}
 </div>
 )}
 ...
 <button onClick={() => removeAct(act.id)} className="text-[9px] text-destructive hover:text-destructive dark:hover:text-destructive opacity-0 group-hover:opacity-100">×</button>
 </div>
 );
 })}
 </div>
 ...
 b.chapter <= currentChapter ? 'bg-success-soft border-success/20 text-success dark:bg-emerald-950/20 ' : 'border-border text-ink-muted'
 }`} title={b.event}>
 {beatEmojis[b.type]} 第{b.chapter}章
 <button onClick={() => removeBeat(i)} className="ml-1 text-ink-subtle hover:text-destructive">×</button>
 </span>
 ))}
 </div>
 )}
