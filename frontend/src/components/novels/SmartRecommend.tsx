...
import { Dna, FileText, Lightbulb } from 'lucide-react';
...
export function SmartRecommend({ genre, chapters }: {
 ...
 <h3 className="font-heading text-base font-semibold text-ink mb-3"><Dna size={12} className="inline" /> 智能题材分析</h3>
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink"><FileText size={12} className="inline" /> 章节标题优化</h3>
 ...
 t.score >= 70 ? 'text-success' : t.score >= 45 ? 'text-warn' : 'text-destructive'
 }`}>{t.score}</span>
 ...
 title={t.suggestion}><Lightbulb size={12} className="inline" /></span>
 )}
 </div>
 </div>
 ))}
