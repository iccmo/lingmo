...
function hookLabel(strength: number): { emoji: string; text: string; color: string } {
 if (strength >= 80) return { emoji: '', text: '强力钩子', color: 'text-success' };
 if (strength >= 60) return { emoji: '🔗', text: '中等钩子', color: 'text-info' };
 if (strength >= 40) return { emoji: '', text: '弱钩子', color: 'text-warn' };
 return { emoji: '', text: '无钩子', color: 'text-destructive' };
}
...
export function EmotionalArc({ chapters }: { chapters?: ChapterMeta[] }) {
 ...
 <AlertTriangle size={12} className='mr-0.5' /> {weakHooks.length} 章钩子偏弱
