...
export function OpeningABTest({ genre, synopsis }: {
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink"><Microscope size={12} className="inline" /> 开局 A/B 测试</h3>
 ...
 (r.scores.overall || 0) >= 0.8 ? 'text-success'
 : (r.scores.overall || 0) >= 0.6 ? 'text-info' : 'text-warn'
 }`}>
...
export function CharacterVoices({ novelId, chapters }: {
 ...
 <div className="flex items-center justify-between mb-3">
 <div>
 <h3 className="font-heading text-base font-semibold text-ink"><Palette size={12} className="inline" /> 角色语音一致性</h3>
 ...
 d.driftScore < 20 ? 'bg-success-soft text-success dark:bg-emerald-900/40 '
 : d.driftScore < 50 ? 'bg-warn-soft text-warn dark:bg-amber-900/40 '
 : 'bg-destructive-soft text-destructive dark:bg-red-900/40 '
 }`}>
 ...
 d.driftScore < 20 ? 'text-success'
 : d.driftScore < 50 ? 'text-warn' : 'text-destructive'
 }`}>
 漂移度 {d.driftScore}%
 </span>
 </div>
