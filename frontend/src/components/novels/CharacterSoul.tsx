...
function CharacterCard({ char, onEdit, onDelete }: {
 ...
 '反派': 'border-l-red-500 bg-destructive-soft/30 dark:bg-red-950/20',
 '导师': 'border-l-emerald-500 bg-success-soft/30 dark:bg-emerald-950/20',
 ...
 <button onClick={onDelete} className="text-[10px] text-ink-muted hover:text-destructive px-1"><Trash2 size={12} className="inline" /></button>
 </div>
 </div>
 ...
 <Zap size={12} className="inline" /> 与 <span className="text-ink font-medium">{char.contrastWith}</span> 形成对比：{char.contrastHow}
 </p>
 )}
 </div>
 );
...
export function CharacterSoul({ novelId }: { novelId: string }) {
 ...
 <p className="text-[10px] font-semibold text-ink-muted uppercase tracking-wide mb-2"><Network size={12} className="inline" /> 角色化学反应</p>
 ...
 <div className="pt-3 border-t border-border">
 <p className="text-[10px] text-success"><CheckCircle2 size={12} className="text-success inline" /> 所有角色都拥有出场·台词·创伤——金庸级角色设计完成</p>
 </div>
 );
 return (
 <div className="pt-3 border-t border-border">
 <p className="text-[10px] text-warn">
 <AlertTriangle size={12} className="text-warn inline" /> {missing.length} 个角色缺少关键要素（出场/台词/创伤至少一项）：
