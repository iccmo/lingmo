...
export function WriterStatusBar({ novelId }: Props) {
  ...
  const gateColor = gate?.gate?.includes('良好') ? 'text-emerald-500'
    : gate?.gate?.includes('注意') ? 'text-amber-500'
    : 'text-red-500';
    ...
    <div className="flex items-center gap-3 mb-3 text-[11px]">
      ...
      {gate && (
        <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full border ${
          gate.gate === '✅ 良好' ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/20'
