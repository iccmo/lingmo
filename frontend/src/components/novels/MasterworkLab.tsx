...
function EmotionalResiduePanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
  ...
  if (!chapterNum) return <p className="text-xs text-ink-muted py-4 text-center">选择章节</p>;
  if (!analysis) return <p className="text-xs text-ink-muted py-4 text-center">无法分析</p>;
  return (<div className="space-y-3"><p className="text-xs text-ink-muted leading-relaxed">读完一章——读者应该在和开头<strong>不一样的情绪状态</strong>里。</p><div className="text-center p-4 rounded-xl bg-gradient-to-br from-accent-soft/30 to-transparent border border-accent/10"><div className={`text-3xl font-bold ${analysis.shift>=6?'text-emerald-500':analysis.shift>=2?'text-amber-500':'text-red-500'}`}>{analysis.shift}</div><div className="text-[10px] text-ink-muted mt-1">情感变化幅度</div></div><div className="grid grid-cols-2 gap-3 text-[10px]"><div className="p-2.5 rounded-lg bg-paper border border-border"><p className="text-ink-subtle mb-1">开头情绪</p>{analysis.beginProfile.filter(e=>e.count>0).slice(0,3).map((e,i)=><span key={i} className="text-ink">{e.emoji}×{e.count} </span>)}</div><div className="p-2.5 rounded-lg bg-paper border border-border"><p className="text-ink-subtle mb-1">结尾情绪</p>{analysis.endProfile.filter(e=>e.count>0).slice(0,3).map((e,i)=><span key={i} className="text-ink">{e.emoji}×{e.count} </span>)}</div></div><div className={`p-3 rounded-lg border text-[10px] ${analysis.endsWithOpenness&&!analysis.endsWithClosure?'bg-emerald-50/30 dark:bg-emerald-950/30 border-emerald-100 dark:border-emerald-800 text-emerald-600 dark:text-emerald-400':analysis.endsWithClosure&&!analysis.endsWithOpenness?'bg-amber-50/30 dark:bg-amber-950/30 border-amber-100 dark:border-amber-800 text-amber-600 dark:text-amber-400':'bg-paper border-border text-ink-muted'}`}>{analysis.endsWithOpenness&&!analysis.endsWithClosure?'✅ 结尾是敞开的——故事结束，人物的生命还在继续。':analysis.endsWithClosure&&!analysis.endsWithOpenness?'🟡 结尾太干净了。神作的结尾不是关上门——是打开窗。':'结尾介于开放和闭合之间。有分寸感。'}</div></div>);
}
