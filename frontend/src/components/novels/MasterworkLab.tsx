...
import { AlertTriangle, CheckCircle2, Search, Telescope, TrendingUp } from 'lucide-react';
...
function WorldLawsPanel({ novelId }: { novelId: string }) {
 ...
 className="text-xs text-ink-muted hover:text-destructive shrink-0">×</button>
 </div>
 ))}
 </div>
 )}
...
function CentralImagePanel({ novelId, chapters }: { novelId: string; chapters?: ChapterMeta[] }) {
 ...
 <div className="p-3 rounded-lg bg-success-soft/30 border border-emerald-100 dark:border-emerald-900/30">
 <p className="text-[10px] font-medium text-success mb-2"><TrendingUp size={12} className="inline" /> 意象含义演化</p>
...
function EchoDetector({ novelId, chapters }: { novelId: string; chapters?: ChapterMeta[] }) {
 ...
 {echoes.length > 0 && (
 <div className="space-y-1.5">
 <p className="text-[10px] text-success font-medium">发现 {echoes.length} 处首尾呼应：</p>
 {echoes.map((e, i) => (
 ...
 <div className="p-3 rounded-lg bg-warn-soft/30 border border-amber-100 dark:border-amber-900/30 text-[10px] text-warn ">
 <AlertTriangle size={12} className="text-warn inline" /> 未检测到明显呼应。试试：让最后一章出现和第一章相同的场景（但意义不同）、同一句话（但说话的人不同）、同一个物件（但含义变了）。
 </div>
 )}
 </div>
 );
...
function QuestionPenetration({ chapters, engineQuestion }: { chapters?: ChapterMeta[]; engineQuestion: string }) {
 ...
 d.level === 'deep' ? 'text-success' : d.level === 'surface' ? 'text-warn' : 'text-ink-subtle'
 }`}>{d.label}</span>
 </div>
 ))}
 </div>
 ...
 ? 'bg-success-soft/30 border-emerald-100 dark:border-emerald-900/30 text-success '
 : absentCount > data.length * 0.3
 ? 'bg-destructive-soft/30 border-red-100 dark:border-red-900/30 text-destructive '
 : 'bg-warn-soft/30 border-amber-100 dark:border-amber-900/30 text-warn '
 }`}>
 {absentCount === 0 && deepCount >= data.length * 0.5
 ? `${deepCount}/${data.length} 章深度触及核心问题。你的小说在追问，不是在填充。`
 : absentCount > data.length * 0.3
 ? `${absentCount}/${data.length} 章完全没有触及核心问题。这些章只是在推进情节——不是在追问。`
...
function ScenePurposePanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 r.grade === 'strong' ? 'bg-success-soft/30 border-emerald-100 dark:border-emerald-900/30' :
 r.grade === 'good' ? 'bg-info-soft/30 dark:bg-sky-950/10 border-sky-100 dark:border-sky-900/30' :
 r.grade === 'ok' ? 'bg-paper border-border' :
 r.grade === 'dead' ? 'bg-destructive-soft/30 border-red-100 dark:border-red-900/30' :
 'bg-warn-soft/30 border-amber-100 dark:border-amber-900/30'
 }`}>
 <div className="flex items-center gap-2 mb-1">
 <span className={`text-xs font-bold ${
 r.grade === 'strong' ? 'text-success' : r.grade === 'good' ? 'text-info' :
 r.grade === 'dead' ? 'text-destructive' : 'text-warn'
 }`}>
 ...
 ? 'bg-success-soft/30 border-emerald-100 text-success'
 : analysis.deadCount > 1
 ? 'bg-destructive-soft/30 border-red-100 text-destructive'
 : 'bg-warn-soft/30 border-amber-100 text-warn'
 }`}>
 {analysis.deadCount === 0 && analysis.strongCount >= analysis.total * 0.3
 ? `${analysis.strongCount}/${analysis.total} 个场景有明确的存在理由`
 : analysis.deadCount > 1
 ? `${analysis.deadCount} 个场景没有冲突/变化/情感——读者会跳过。问自己：删掉这一段，故事缺了什么？`
...
function RereadValuePanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 <div className={`text-3xl font-bold ${analysis.score >= 60 ? 'text-success' : analysis.score >= 30 ? 'text-warn' : 'text-destructive'}`}>
 {analysis.score}
 </div>
 ...
 <div>
 <p className="text-[10px] font-medium text-ink-muted mb-1.5"><Search size={12} className="inline" /> 隐藏线索（第一遍容易忽略）</p>
 ...
 <div>
 <p className="text-[10px] font-medium text-ink-muted mb-1.5"><Telescope size={12} className="inline" /> 双层含义（重读时才明白）</p>
 ...
 <div className="p-3 rounded-lg bg-warn-soft/30 border border-amber-100 text-[10px] text-warn">
 <AlertTriangle size={12} className="text-warn inline" /> 未检测到隐藏线索或双层含义。试试：在对话中埋一个只有知道结局才懂的暗示。让某个角色说一句"当时以为是玩笑"的话。
 </div>
 )}
...
function EmotionLayerPanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 <div className={`text-2xl font-bold ${analysis.totalActive >= 4 ? 'text-success' : analysis.totalActive >= 2 ? 'text-warn' : 'text-destructive'}`}>
 {analysis.totalActive}/8
 </div>
 <div className="text-[10px] text-ink-muted mt-1">
 {analysis.totalActive >= 4 ? '情感层次丰富' : analysis.totalActive >= 2 ? '🟡 情感层次一般' : '情感层次单一'}
 </div>
 </div>
...
function ThemeDeepenPanel({ content, chapterNum, checking, engineQuestion }: { content: string; chapterNum: number | null; checking: boolean; engineQuestion: string }) {
 ...
 else insight = '这一章绕开了核心问题。情节在推进，但问题在等待。';
 ...
 {/* Score */}
 <div className="text-center p-4 rounded-xl bg-gradient-to-br from-accent-soft/30 to-transparent border border-accent/10">
 <div className={`text-3xl font-bold ${analysis.totalScore >= 60 ? 'text-success' : analysis.totalScore >= 30 ? 'text-warn' : 'text-destructive'}`}>
 {analysis.totalScore}
 </div>
 ...
 <div className="p-2.5 rounded-lg bg-success-soft/30 border border-emerald-100 text-center">
 <div className="font-bold text-success">{analysis.complicates}</div>
 <div className="text-ink-subtle">转折词</div>
 <div className="text-[9px] text-ink-subtle mt-0.5">让问题变复杂</div>
 </div>
 <div className="p-2.5 rounded-lg bg-info-soft/30 dark:bg-sky-950/10 border border-sky-100 text-center">
 <div className="font-bold text-info">{analysis.deepens}</div>
 <div className="text-ink-subtle">深化词</div>
 <div className="text-[9px] text-ink-subtle mt-0.5">推进理解</div>
 </div>
 <div className="p-2.5 rounded-lg bg-warn-soft/30 border border-amber-100 text-center">
 <div className="font-bold text-warn">{analysis.restates}</div>
...
function CompressionLossPanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 return (<div className="space-y-3"><p className="text-xs text-ink-muted leading-relaxed">神作不能被情节总结替代。<strong>情节是骨架，但灵魂在骨架之间的空隙里。</strong></p><div className="text-center p-4 rounded-xl bg-gradient-to-br from-accent-soft/30 to-transparent border border-accent/10"><div className={`text-3xl font-bold ${analysis.score>=60?'text-success':analysis.score>=30?'text-warn':'text-destructive'}`}>{analysis.textureRatio}%</div><div className="text-[10px] text-ink-muted mt-1">不可压缩的「肌理」占比</div></div><div className="p-3 rounded-lg bg-paper border border-border text-[10px]"><p className="text-ink-muted mb-1">只用三句话总结本章情节：</p>{analysis.plotLines.slice(0,3).map((l,i)=><p key={i} className="text-ink leading-relaxed">{i+1}. {l.slice(0,60)}</p>)}<p className="text-ink-subtle mt-2">{analysis.textureRatio<30?'这三句话涵盖了70%——剩下的在填充而非丰富。':analysis.textureRatio>=60?'大量细节、氛围、潜台词无法压缩——这就是不可压缩性。':'🟡 有一定肌理，但仍有不少可压缩段落。'}</p></div></div>);
}
...
function CharacterMercyPanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 if (!chapterNum) return <p className="text-xs text-ink-muted py-4 text-center">选择章节</p>;
 if (!analysis) return <p className="text-xs text-ink-muted py-4 text-center">无法分析</p>;
 return (<div className="space-y-3"><p className="text-xs text-ink-muted leading-relaxed">托尔斯泰写安娜的丈夫——被戴绿帽的官僚——你恨不起来。<strong>神作的作者对所有角色都有慈悲。</strong></p><div className="grid grid-cols-3 gap-2 text-center text-[10px]">{[{k:'脆弱性',v:analysis.vulnerability,i:'😢',j:'🪨',t:'有脆弱时刻',f:'刀枪不入'},{k:'动机',v:analysis.motivation,i:'🤔',j:'👹',t:'有合理动机',f:'纯粹邪恶'},{k:'理解',v:analysis.understanding,i:'',j:'🔨',t:'尝试理解',f:'直接审判'}].map(m=><div key={m.k} className={`p-2.5 rounded-lg border ${m.v?'bg-success-soft/30 border-emerald-100':'bg-destructive-soft/30 border-red-100'}`}><div className="text-lg">{m.v?m.i:m.j}</div><div className="text-ink font-medium mt-1">{m.k}</div><div className="text-ink-subtle">{m.v?m.t:m.f}</div></div>)}</div><div className={`p-3 rounded-lg border text-[10px] ${analysis.level==='慈悲'?'bg-success-soft/30 dark:bg-emerald-950/30 border-emerald-100 text-success ':analysis.level==='公允'?'bg-warn-soft/30 dark:bg-amber-950/30 border-amber-100 text-warn ':'bg-destructive-soft/30 dark:bg-red-950/30 border-red-100 text-destructive '}`}>{analysis.level==='慈悲'?'角色具有脆弱性、合理动机和叙事理解——读者能看到每个人为什么成为现在的样子。':analysis.level==='公允'?'🟡 有一定角色维度，但还可以更深。试试给反派一个读者能共情的动机。':'角色过于刻板。每个反派都应该有"那个人也很可怜"的瞬间。'}</div></div>);
}
...
function EmotionalResiduePanel({ content, chapterNum, checking }: { content: string; chapterNum: number | null; checking: boolean }) {
 ...
 if (!chapterNum) return <p className="text-xs text-ink-muted py-4 text-center">选择章节</p>;
 if (!analysis) return <p className="text-xs text-ink-muted py-4 text-center">无法分析</p>;
 return (<div className="space-y-3"><p className="text-xs text-ink-muted leading-relaxed">读完一章——读者应该在和开头<strong>不一样的情绪状态</strong>里。</p><div className="text-center p-4 rounded-xl bg-gradient-to-br from-accent-soft/30 to-transparent border border-accent/10"><div className={`text-3xl font-bold ${analysis.shift>=6?'text-success':analysis.shift>=2?'text-warn':'text-destructive'}`}>{analysis.shift}</div><div className="text-[10px] text-ink-muted mt-1">情感变化幅度</div></div><div className="grid grid-cols-2 gap-3 text-[10px]"><div className="p-2.5 rounded-lg bg-paper border border-border"><p className="text-ink-subtle mb-1">开头情绪</p>{analysis.beginProfile.filter(e=>e.count>0).slice(0,3).map((e,i)=><span key={i} className="text-ink">{e.emoji}×{e.count} </span>)}</div><div className="p-2.5 rounded-lg bg-paper border border-border"><p className="text-ink-subtle mb-1">结尾情绪</p>{analysis.endProfile.filter(e=>e.count>0).slice(0,3).map((e,i)=><span key={i} className="text-ink">{e.emoji}×{e.count} </span>)}</div></div><div className={`p-3 rounded-lg border text-[10px] ${analysis.endsWithOpenness&&!analysis.endsWithClosure?'bg-success-soft/30 dark:bg-emerald-950/30 border-emerald-100 text-success ':analysis.endsWithClosure&&!analysis.endsWithOpenness?'bg-warn-soft/30 dark:bg-amber-950/30 border-amber-100 text-warn ':'bg-paper border-border text-ink-muted'}`}>{analysis.endsWithOpenness&&!analysis.endsWithClosure?'结尾是敞开的——故事结束，人物的生命还在继续。':analysis.endsWithClosure&&!analysis.endsWithOpenness?'🟡 结尾太干净了。神作的结尾不是关上门——是打开窗。':'结尾介于开放和闭合之间。有分寸感。'}</div></div>);
}
...
function FormSoulUnityPanel({ content, chapterNum, checking, novelId }: { content: string; chapterNum: number | null; checking: boolean; novelId: string }) {
 ...
 return (<div className="space-y-3"><p className="text-xs text-ink-muted leading-relaxed">神作的<strong>形式就是内容</strong>。换了形式，内容就消失了。</p><div className="text-center p-4 rounded-xl bg-gradient-to-br from-accent-soft/30 to-transparent border border-accent/10"><div className={`text-3xl font-bold ${(analysis.score??0)>=70?'text-success':(analysis.score??0)>=40?'text-warn':'text-destructive'}`}>{analysis.score??0}</div><div className="text-[10px] text-ink-muted mt-1">形式-灵魂统一度</div></div>{(analysis.matches??[]).map((m,i)=><p key={i} className="text-[10px] text-success "><CheckCircle2 size={12} className="text-success inline" /> {m}</p>)}{(analysis.mismatches??[]).map((m,i)=><p key={i} className="text-[10px] text-destructive"><AlertTriangle size={12} className="text-warn inline" /> {m}</p>)}</div>);
}
