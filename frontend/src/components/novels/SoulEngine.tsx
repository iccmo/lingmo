...
const POLARITIES: (Polarity & { category: string })[] = [
  ...
  { category: 'metaphysical', id: 'freedom-fate', emoji: '⚔️', name: '自由 ↔ 命运', poles: ['我要选择', '我已注定'], question: '如果你最自由的选择——恰好印证了你逃不掉的命运——你是自由的还是被困的？', prompt: '每一章让角色做一个选择。但选择的后果不是他们能控制的。自由的行动导致宿命的结果——这是最深的悲剧。', deadlySins: ['命运可轻松改变','自由选择无后果','宿命让角色放弃努力'], masters: [{author:'金庸',work:'《天龙八部》',note:'乔峰的命运早在血统中注定'},{author:'索福克勒斯',work:'《俄狄浦斯王》',note:'逃避预言恰好实现预言'},{author:'马尔克斯',work:'《百年孤独》',note:'每一代都反抗命运也都重复命运'}] },
  { category: 'metaphysical', id: 'life-death', emoji: '🕯️', name: '生 ↔ 死', poles: ['活着', '死去'], question: '如果死亡是唯一的确定性——活着的时候什么是值得的？', prompt: '死亡应该是每一个重大决定的背景。不是因为角色要死——是因为读者知道人会死。在有限的时间里，每个选择都有重量。', deadlySins: ['死亡当情节工具','角色对死亡无感','活着被当理所当然'], masters: [{author:'余华',work:'《活着》',note:'所有人都死了——活着本身就是答案'},{author:'托尔斯泰',work:'《伊凡·伊里奇之死》',note:'临死才发现一生没真正活过'},{author:'海明威',work:'《老人与海》',note:'老人可能死在海上——但他必须去'}] },
  ...
  { category: 'metaphysical', id: 'order-chaos', emoji: '🌪️', name: '秩序 ↔ 混沌', poles: ['想要秩序', '世界是混沌'], question: '你精心构建的秩序——会不会本身就是一种幻觉？', prompt: '建立秩序，然后让它瓦解。不是一次性的——是反复的。角色越是努力建立秩序，混沌就越是以意想不到的方式回来。', deadlySins: ['秩序永远获胜','混沌被完全驯服'], masters: [{author:'戈尔丁',work:'《蝇王》',note:'一群孩子试图建立秩序——最后变成猎杀'},{author:'马尔克斯',work:'《百年孤独》',note:'马孔多被历史反复摧毁'},{author:'刘震云',work:'《一句顶一万句》',note:'一句话建立秩序，另一句话崩塌'}] },
  ...
  { category: 'inner', id: 'hope-despair', emoji: '🕳️', name: '希望 ↔ 绝望', poles: ['还有明天', '没有明天'], question: '如果希望本身是一种残忍——你还要不要抱有希望？', prompt: '不要让角色轻易找到希望。也不要把绝望写成终点。真正的力量在于：明明知道可能没有明天，今天还是做了该做的事。', deadlySins: ['轻率的希望','绝望被当作答案','角色从未真正怀疑过'], masters: [{author:'贝克特',work:'《等待戈多》',note:'戈多不来——但他们还在等'},{author:'加缪',work:'《西西弗神话》',note:'推石头上山——知道它会滚下来——继续推'},{author:'科马克·麦卡锡',work:'《路》',note:'世界末日。父亲对孩子说：你是好人。'}] },
  ...
  return (
    <div className="p-4 bg-card border border-border rounded-xl">
      <div className="flex items-center justify-between mb-3">
        ...
        {fp && (
          <button onClick={clear} className="text-[10px] text-ink-muted hover:text-red-500 transition-colors">清除</button>
        )}
      </div>
      ...
      {polarity && !fp && (
        <div className="space-y-3 animate-[fadeSlideIn_0.2s_ease-out]">
          ...
          <div className="p-3 rounded-lg bg-red-50/50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30">
            <p className="text-[10px] font-semibold text-red-500 uppercase tracking-wide mb-1">⚠ 如果你这样做，张力就消失了</p>
            {polarity.deadlySins.map((s, i) => (
              <p key={i} className="text-[11px] text-red-600 dark:text-red-400">✕ {s}</p>
            ))}
          </div>
