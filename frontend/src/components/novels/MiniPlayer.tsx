    ...
    <div className="fixed z-50 bg-card border border-border rounded-xl shadow-2xl w-[360px] max-md:w-[320px] max-sm:w-[calc(100vw-16px)] max-sm:left-2 max-sm:right-2 animate-[fadeSlideIn_0.2s_ease-out]"
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border cursor-move"
        ...
        <div className="flex items-center gap-0.5 shrink-0 ml-1">
          {radioMode && <span className="text-[9px] text-red-400 dark:text-red-300 animate-pulse">📻</span>}
          {autoContinue && !radioMode && <span className="text-[8px] text-accent">🔄</span>}
          ...
          <button onClick={() => { stop(); setVisible(false); }} className="text-[10px] text-ink-muted hover:text-red-500 px-1">✕</button>
        </div>
      </div>
      ...
      {!loading && (
        <div className="flex items-center gap-2 px-3 mt-1">
          ...
          <button onClick={() => previewVoice(voice)}
            className="text-[9px] px-1.5 py-1 rounded border border-border text-ink-muted hover:text-accent shrink-0">▶️</button>
      ...
      <div className="flex items-center justify-center gap-1.5 px-3 pb-1.5">
        <button onClick={() => rewind30s()} disabled={loading || !playing}
          className="text-[9px] px-2 py-0.5 rounded border border-border text-ink-muted hover:text-ink disabled:opacity-20" title="回退30秒">↩️30s</button>
        ...
        <button onClick={stop} className="text-[9px] px-2 py-0.5 rounded border border-red-200 text-red-400 hover:text-red-600 dark:border-red-800">⏹ 停止</button>
        <button onClick={() => addBookmark()} disabled={loading}
          className="text-[9px] px-2 py-0.5 rounded border border-amber-200 text-amber-500 hover:text-amber-600 dark:border-amber-800 disabled:opacity-20">🔖 标记</button>
          ...
          <div className="px-3 pb-2 space-y-2">
            ...
            <div className="flex items-center gap-1 flex-wrap">
              <button onClick={toggleRadioMode} title="跨书随机连播 · 像听广播"
                className={`text-[9px] px-2 py-0.5 rounded border ${radioMode ? 'bg-red-100 dark:bg-red-950/30 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800' : 'border-border text-ink-muted hover:text-ink'}`}>📻 电台</button>
              <button onClick={toggleDramaticMode} title="单人多声线 · 自动变调演绎角色"
                className={`text-[9px] px-2 py-0.5 rounded border ${dramaticMode ? 'bg-purple-100 dark:bg-purple-950/30 text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-800' : 'border-border text-ink-muted hover:text-ink'}`}>🎭 角色</button>
            ...
            <div>
              ...
              <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                {([ [null, '关'], ['rain', '🌧️雨'], ['thunder', '⛈️雷'], ['campfire', '🔥火'], ['ocean', '🌊海'], ['forest', '🌲林'], ['wind', '💨风'], ['cafe', '☕厅'], ['white', '⬜白'] ] as const).map(([key, label]) => {
            ...
            {/* Background Music */}
            <div>
              <span className="text-[9px] text-ink-subtle">🎵 背景音乐</span>
              <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                {([ [null, '关'], ['peaceful', '😌宁静'], ['tense', '😰紧张'], ['epic', '⚔️史诗'], ['melancholy', '😢忧伤'] ] as const).map(([key, label]) => {
                  const tips: Record<string, string> = {
      ...
      {sleepTimer > 0 && sleepRemaining > 0 && (
        <div className="mx-3 px-2 py-1 mb-1 rounded bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 flex items-center justify-between text-[9px]">
          <span className="text-amber-600 dark:text-amber-400">⏰ {sleepRemaining <= 30 ? '🔉 渐弱… ' : ''}{fmtCountdown(sleepRemaining)}后暂停</span>
          <button onClick={cancelSleepTimer} className="text-amber-500 hover:text-amber-700 dark:hover:text-amber-300 ml-2 shrink-0">取消</button>
        </div>
      )}
            ...
            playlist.map((item, i) => {
              ...
              return (
                <div key={`${item.novelId}-${item.chapterNum}`} onClick={() => playChapter(item)}
                  ...
                  <button onClick={e => { e.stopPropagation(); removeFromPlaylist(i); }} className="text-ink-subtle hover:text-red-500 shrink-0">×</button>
                </div>
              );
            })
          )}
      ...
      <div className="flex items-center justify-between px-3 py-1 border-t border-border">
        <div className="flex items-center gap-1">
          <button onClick={() => { setShowPlaylist(!showPlaylist); setShowHistory(false); }}
            className={`text-[9px] px-2 py-0.5 rounded border ${showPlaylist ? 'bg-accent/10 border-accent/30 text-accent' : 'border-border text-ink-muted hover:text-ink'}`}>📋 ({playlist.length})</button>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-2 border-t border-border pt-2">
          ...
          <button onClick={() => {
            ...
            📖 打开《{current.novelTitle.slice(0, 8)}》第{current.chapterNum}章 →
          </button>
