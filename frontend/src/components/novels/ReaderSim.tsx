...
const WHAT_IF_SCENARIOS = [
 ...
 { label: '外部灾难降临', key: 'disaster', desc: '如果天灾/意外事件在这一章发生...' },
];
...
export function ReaderSim({ novelId, chapters }: {
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink">读者留存模拟</h3>
 <p className="text-[11px] text-ink-muted">模拟读者逐章阅读的留存率</p>
 </div>
 <div className="flex gap-2 text-xs">
 <div className={`text-center px-2 py-1 rounded ${
 finalRetention >= 50 ? 'bg-success-soft text-success dark:bg-emerald-900/20 '
 : finalRetention >= 25 ? 'bg-warn-soft text-warn '
 : 'bg-destructive-soft text-destructive dark:bg-red-900/20 '
 }`}>
 ...
 {retention.map((r, _i) => {
 const barColor = r.dropRisk >= 50 ? 'bg-red-400 dark:bg-destructive-soft0'
 : r.dropRisk >= 25 ? 'bg-amber-400 dark:bg-warn-soft0'
 : 'bg-emerald-400 dark:bg-success-soft0';
 const bgColor = r.dropRisk >= 50 ? 'bg-destructive-soft dark:bg-red-950'
 : r.dropRisk >= 25 ? 'bg-warn-soft dark:bg-amber-950' : '';
 ...
 {r.dropRisk >= 50 && (
 <span className="text-[10px] text-destructive shrink-0" title={r.reasons.join(', ')}><AlertTriangle size={12} className="text-warn inline" /></span>
 )}
 </div>
 );
 })}
 ...
 <p className="text-[11px] text-warn font-medium mb-1">
 <AlertTriangle size={12} className="text-warn inline" /> {dangerChapters.length} 个高风险流失点：
 </p>
