...
import { Network, CheckCircle2, Telescope } from 'lucide-react';
...
const ROLE_COLORS: Record<string, string> = {
 '主角': '#92400E', '反派': '#DC2626', '导师': '#059669', '配角': '#B45309', '路人': '#9CA3AF',
};
...
export function PlotNetwork({ novelId }: { novelId: string }) {
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink"><Network size={16} className='mr-1.5 text-accent' /> 情节网络</h3>
 ...
 {pts.map((p, i) => (
 <span key={i} className={p.is_resolved ? 'text-success' : 'text-warn'}>
 {p.is_resolved ? <CheckCircle2 size={12} className='text-success' /> : <Telescope size={12} className='text-accent' />} {p.content.slice(0, 30)}{i < pts.length - 1 ? ' · ' : ''}
 </span>
 ))}
 </div>
 )}
