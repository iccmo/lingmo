...
export function PlatformChecklist({ chapters, genre }: {
 ...
 <div>
 <h3 className="font-heading text-base font-semibold text-ink"><ClipboardList size={12} className="inline" /> 发布前检查</h3>
 ...
 : criticalFail > 0
 ? 'bg-destructive-soft text-destructive dark:bg-red-900/30 '
 : 'bg-warn-soft text-warn dark:bg-amber-900/30 '
 }`}>
 ...
 !item.pass && item.weight === 'critical' ? 'bg-destructive-soft/50 dark:bg-red-950/20' : ''
 }`}>
 <span className="shrink-0 mt-0.5">{item.pass ? '' : item.weight === 'critical' ? '' : ''}</span>
 ...
 <span className="text-[10px] text-destructive font-semibold shrink-0">必须修复</span>
 )}
 </div>
 ))}
 </div>
