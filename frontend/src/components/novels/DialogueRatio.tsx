...
function riskBadge(risk: ChapterDialogueStats['risk']): {
 ...
 if (risk === 'info-dump') return { text: '信息倾泻', color: 'text-destructive ' };
 if (risk === 'talking-heads')
 return { text: '对话过多', color: 'text-warn ' };
 return null;
}
...
export function DialogueRatio({ chapters, contentMap }: Props) {
 ...
 <div className="flex items-center gap-1.5">
 <span className="w-2 h-2 rounded-full inline-block bg-warn-soft0" />
 <span className="text-ink-muted">信息倾泻风险 (&gt;80% 叙述)</span>
 </div>
 <div className="flex items-center gap-1.5">
 <span className="w-2 h-2 rounded-full inline-block bg-destructive-soft0" />
 ...
 {infoDumps.length > 0 && (
 <div className="text-[11px] text-warn bg-warn-soft/50 dark:bg-amber-950/20 border border-warn/20 rounded-lg px-3 py-2">
 ...
 <div className="text-[11px] text-destructive bg-destructive-soft/50 dark:bg-red-950/20 border border-destructive/20 rounded-lg px-3 py-2">
