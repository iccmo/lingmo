...
export function QualityTrend({ chapters }: Props) {
 ...
 {gen.length >= 4 && (
 <span className={wordTrend > 10 ? 'text-success' : wordTrend < -10 ? 'text-warn' : ''}>
 ...
 const gradeColor = grade === 'A' ? 'text-success' : grade === 'B' ? 'text-info' : grade === 'C' ? 'text-warn' : 'text-destructive';
 return (
