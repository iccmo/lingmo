...
export function ProducePanel({ novelId, chapters }: Props) {
 ...
 i < currentStep
 ? 'bg-success-soft0/10 text-success border border-emerald-500/20'
 : i === currentStep
 ...
 <div className="flex items-center justify-center gap-3 py-3 bg-destructive-soft0/5 border border-red-500/10 rounded-lg">
 <span className="text-xs text-destructive">{status.message}</span>
 <button
 onClick={handleProduce}
 className="px-2 py-1 text-[10px] font-medium text-destructive border border-red-500/20 rounded hover:bg-destructive-soft0/10 transition"
 >
 重试
 </button>
 </div>
 ...
 <div className="text-3xl mb-3"><Film size={12} className="inline" /></div>
