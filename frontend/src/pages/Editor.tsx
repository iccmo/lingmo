...
export function Editor() {
 ...
 <Sparkles size={13} className='mr-1' /> 生成 3 个草稿方向
 </button>
 </div>
 )}
 ...
 {d.hook && (
 <p className="text-[10px] text-accent mt-2"><Anchor size={10} className='mr-0.5' /> 钩子: {d.hook.slice(0, 50)}</p>
 )}
 </CardContent>
 </Card>
 ))}
 ...
 <div className={`h-full rounded-full transition-all ${wordPct >= 100 ? 'bg-success-soft0' : 'bg-accent'}`}
 style={{ width: `${Math.max(3, wordPct)}%` }} />
 </div>
 <span className={`tabular-nums ${wordCount >= wordGoal ? 'text-success font-semibold' : 'text-ink-muted'}`}>
 ...
 {showPreview ? '隐藏预览' : <><Eye size={13} className='mr-1' /> 预览</>}
 </button>
 </div>
 </div>
