          ...
          {audioLoading ? (
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-3 mt-3">
                ...
                <button onClick={stop} className="text-sm p-1 text-red-400 hover:text-red-600 dark:hover:text-red-400">⏹</button>
                <button onClick={() => skipChapter(1)} className="text-sm p-1 text-ink-muted hover:text-ink">⏭</button>
                <button onClick={() => addBookmark()} className="text-sm p-1 text-amber-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors" title="标记当前位置">
                  🔖
                </button>
              </div>
                              ...
                              <div className="max-h-[200px] overflow-y-auto space-y-0.5">
                                {expandedChapters.map(ch => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <span className="shrink-0">{isCurrent && playing ? '🔊' : icon || '🎵'}</span>
          ...
          {playlist.length > 0 && (
            <div className="border-t border-border pt-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={clearPlaylist} className="text-[10px] text-red-400 hover:text-red-600 dark:hover:text-red-400">清空</button>
              </div>
              <div className="space-y-0.5 max-h-[300px] overflow-y-auto">
                {playlist.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="shrink-0">{isCurrent && playing ? '🔊' : '🎵'}</span>
        ...
        <div className="space-y-4">
          ...
          {bookmarks.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-xs text-ink-muted mt-1">播放中点 🔖 按钮，标记精彩片段</p>
            </div>
          ) : (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { localStorage.removeItem('audio-bookmarks'); setBookmarks([]); toast.success('已清空'); }}
                  className="text-[10px] text-red-400 hover:text-red-600 dark:hover:text-red-400">清空</button>
              </div>
              {bookmarks.map(bm => (
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1 shrink-0 ml-2">
                      ...
                      <button onClick={() => { removeBookmark(bm.id); setBookmarks(getBookmarks()); }}
                        className="text-[10px] text-ink-subtle hover:text-red-500">×</button>
                    </div>
                  </div>
                </div>
              ))}
