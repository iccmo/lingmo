    ...
    <div className="page-enter">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          <Card className="border-border">
            <CardContent className="p-4 text-center">
              <div className="font-heading text-[28px] font-semibold text-amber-500">{audit.total_open}</div>
              <div className="text-[11px] text-ink-muted">未回收伏笔</div>
            </CardContent>
          </Card>
          <Card className="border-border">
            <CardContent className="p-4 text-center">
              <div className="font-heading text-[28px] font-semibold text-emerald-500">{audit.total_resolved}</div>
      ...
      {audit?.warning && (
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg dark:bg-amber-900/20 dark:border-amber-800">
          <p className="text-xs text-amber-700 dark:text-amber-400 font-medium">⚠️ {audit.warning}</p>
          {audit.stale.length > 0 && (
            <div className="mt-2 space-y-1">
              {audit.stale.map((s, i) => (
                <div key={i} className="text-[11px] text-amber-600 dark:text-amber-500 flex gap-2">
                  ...
                  <CardContent className="p-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-2 mt-1.5">
                          <Badge variant="outline" className="text-[10px]">{p.type || '伏笔'}</Badge>
                          <span className="text-[10px] text-emerald-500">已回收</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
      ...
      <Card className="mt-6 border-border bg-muted/30">
        <CardContent className="p-4">
          <h3 className="font-heading text-base font-semibold text-ink mb-2">💡 伏笔管理技巧</h3>
          <div className="grid grid-cols-3 gap-3 text-xs text-ink-muted">
            <div>
              <p className="font-medium text-ink mb-0.5">📌 三章原则</p>
            ...
            <div>
              <p className="font-medium text-ink mb-0.5">⚠️ 10章警戒线</p>
