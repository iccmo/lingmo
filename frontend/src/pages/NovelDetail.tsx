...
export function NovelDetail({ mode }: Props) {
  ...
  function buildGenContext(novelId: string, userDirection: string): string {
    ...
    const allRules = [
      ...
      { rule: '禁止用破折号做停顿。全文破折号不超过3个。段落过渡靠节奏，不靠标点。', always: true },
      { rule: '对话标签克制。80%的对话只用「说」「问」或不用标签。偶尔用「喊道」「低声道」——但不要「他冷冷地说」「她惊讶地回应」。对话本身有力量，副词在侮辱读者的理解力。', always: true },
      { rule: '减少思想汇报。「他心想」「她意识到」每章最多3处。删除多余的心理描写——用动作、细节、沉默替代。让读者自己推断角色在想什么。', always: true },
      { rule: '少用时间词制造紧张。「就在这时」「突然」「此刻」不是不能写——但每章不超过2次。紧张感来自事件本身，不靠时间词提醒。', always: true },
