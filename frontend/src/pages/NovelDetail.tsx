...
import {
 Zap, BookOpen, CheckCircle2, AlertTriangle, Trophy,
 Palette, MessageSquare, PenLine, Send, Copy, ClipboardList, Bot,
 Lightbulb, FileText, Search, BarChart3, Timer, Network,
 Smartphone, Loader2, FileDown,
} from 'lucide-react';
...
export function NovelDetail({ mode }: Props) {
 ...
 const allRules = [
 { rule: '展示，不告诉。三项硬指标：①情绪用身体反应（胃收紧/牙打颤），不写「他感到紧张」②对话标签80%用「说」「问」或不用，禁止加副词（不要「冷冷地说」）③「他心想/她意识到」每章最多3处。', always: true },
 { rule: '用错的比喻才是好比喻。不要写「眼睛像鹰一样锐利」——写有偏差的、让读者记住的比喻。', always: true },
 { rule: '禁止用破折号做停顿。全文破折号不超过3个。少用「就在这时/突然/此刻」——每章最多2次。', always: true },
 { rule: '世界透过眼睛看。环境描写不是客观的——是角色此刻看到的。愤怒的人看到的雨和恋爱的人看到的雨不是同一场雨。环境带着情绪，情绪印在环境上。', always: true },
 ...
 <span className="flex items-center gap-1.5"><BookOpen size={12} className="text-accent" />上次：第{ch.number}章「{ch.title}」</span>
 <button onClick={() => {
 ...
 <Search size={14} className="mr-1" /> 角色追踪
 </button>
 );
 } catch { return null; }
 })()}
 ...
 {loadingCover ? <><Loader2 size={13} className='animate-spin' /> 生成中...</> : <><Palette size={13} /> 生成封面</>}
 </button>
 {coverPrompt && (
 <span className="text-[10px] text-ink-subtle truncate max-w-[300px]" title={coverPrompt}>
 <MessageSquare size={12} className="mr-1" /> {coverPrompt.slice(0, 60)}{coverPrompt.length > 60 ? '...' : ''}
 </span>
 )}
 </div>
 {coverSvg && (
 ...
 ? 'text-destructive font-semibold hover:underline cursor-pointer'
 : 'text-warn hover:underline cursor-pointer'}>
 <AlertTriangle size={12} className='mr-0.5' /> {a.msg.slice(0,40)}
 </button>
 ) : (
 <span key={i} className={a.level==='critical'?'text-destructive font-semibold':'text-warn '}><AlertTriangle size={12} className='mr-0.5' /> {a.msg.slice(0,40)}</span>
 );
 })}
 {cockpit.next_actions?.slice(0,2).map((a,i)=><span key={i} className="text-ink-muted">→ {a}</span>)}
 {cockpit.milestones?.slice(0,1).map((m,i)=><span key={i} className="text-success"><Trophy size={12} className="mr-0.5" /> 还需{m.need}章到{m.reward}</span>)}
 {cockpit.revenue_projection && <span className="text-success ml-auto">{cockpit.revenue_projection}</span>}
 </div>
 )}
 ...
 <span className="text-ink font-medium"><BookOpen size={12} className="mr-1" /> {act.name}</span>
 ...
 <Zap size={12} className="mr-0.5" /> {b.chapter - currentCh}章后：{b.event.slice(0, 15)}
 </span>
 ))}
 </div>
 )}
 ...
 step.done ? 'bg-success-soft/30 border-success/20 ' : 'bg-card border-border hover:border-accent/30'
 }`}>
 <div className="flex items-center gap-2 mb-1">
 <span className="text-lg">{step.icon}</span>
 <span className={`text-xs font-semibold ${step.done ? 'text-success ' : 'text-ink'}`}>
 {step.done ? <CheckCircle2 size={12} className='text-success' /> : <span className='w-3 h-3 rounded-full border border-border inline-block' />} {step.label}
 </span>
 </div>
 ...
 <h3 className="text-xs font-semibold text-ink-muted uppercase tracking-wide"><ClipboardList size={13} className="mr-1" /> 写作面板</h3>
 ...
 <div className="flex items-center justify-between mb-3">
 <h3 className="font-heading text-base font-semibold text-ink"><Timer size={16} className="mr-1.5 text-accent" /> 故事时间线</h3>
 ...
 <h3 className="font-heading text-base font-semibold text-ink mb-3"><Network size={16} className="mr-1.5 text-accent" /> 角色关系图</h3>
 <CharacterGraph
 ...
 <span className="font-semibold text-ink"><BarChart3 size={13} className="mr-1" /> 本次写作会话</span>
 ...
 <div>
 <div className={`font-heading text-lg font-bold ${sessionScores.length > 0 ? (sessionScores.reduce((a,b)=>a+b,0)/sessionScores.length >= 0.8 ? 'text-success' : 'text-warn') : 'text-ink'}`}>
 ...
 <div className="p-3 rounded-lg bg-success-soft/50 dark:bg-emerald-950/20 border border-success/20 flex items-center gap-3 text-xs animate-[fadeSlideIn_0.3s_ease-out]">
 <CheckCircle2 size={18} className="text-success" />
 <div className="flex-1">
 <span className="text-success dark:text-emerald-300 font-medium">章节已生成</span>
 ...
 <PenLine size={12} className="mr-1" /> 记录因果（本章有什么动作会在后续产生涟漪？）
 </button>
 ) : (
 ...
 <h3 className="font-heading text-lg font-semibold text-ink mb-1"><BookOpen size={13} className="mr-1" /> 批量生成</h3>
 ...
 <Button size="sm" variant="outline" className="text-destructive border-red-300 hover:bg-destructive-soft dark:hover:bg-red-950"
 onClick={async () => {
 ...
 <h4 className="text-xs font-semibold text-ink mb-3"><Bot size={13} className="mr-1" /> 自动生成配置</h4>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
 <div>
 ...
 ? '⏳ 生成中...'
 : <><Zap size={13} /> 生成下一章 <span className="text-[9px] opacity-50 ml-1">Ctrl+G</span></>
 }
 </Button>
 <Button size="sm" variant="outline" className="text-xs"
 onClick={handleQuickGenerate} title="跳过对话框直接生成: Shift+G"
 disabled={!!genStatus && genStatus.status !== 'error' && genStatus.status !== 'complete'}>
 <Zap size={13} /> 快速生成 <span className="text-[9px] opacity-50 ml-1">Shift+G</span>
 </Button>
 <Button size="sm" variant="outline" className="text-xs"
 onClick={() => setShowBatchDialog(true)} title="批量生成多章"
 disabled={!!genStatus && genStatus.status !== 'error' && genStatus.status !== 'complete'}>
 <BookOpen size={13} className="mr-1" /> 批量生成
 </Button>
 {batchStatus && batchStatus.status !== 'idle' && (
 <span className="text-[10px] text-ink-muted self-center px-2 py-1 bg-paper rounded">
 <BookOpen size={13} className='mr-1' /> {batchStatus.status === 'queued' ? '排队中' : batchStatus.status === 'running' ? `生成中 ${batchStatus.progress.current}/${batchStatus.progress.total}` : batchStatus.status === 'done' ? '完成' : '失败'}
 </span>
 )}
 <Button size="sm" variant="outline" onClick={() => { setCloneTitle(novel?.title + '（副本）' || ''); setCloneGenre(novel?.genre || '玄幻'); setCloneName(''); setShowClone(true); }}><Copy size={13} className="mr-1" /> 复制开新书</Button>
 <Button size="sm" variant="outline" onClick={() => navigate(`/novels/${id}/world`)}>🌍 世界观编辑器</Button>
 <Button size="sm" variant="outline" onClick={() => navigate(`/novels/${id}/outline`)}><ClipboardList size={13} className="mr-1" /> 章节大纲</Button>
 <Button size="sm" variant="outline" onClick={handleSuggestOutline} disabled={loadingSuggestions}>
 {loadingSuggestions ? <><Loader2 size={13} className='animate-spin' /> 分析中...</> : <><Lightbulb size={13} /> 大纲建议</>}
 </Button>
 ...
 <Button size="sm" variant="outline" onClick={() => navigate(`/novels/${id}/edit`)}><PenLine size={13} className='mr-1' /> 创作者模式</Button>
 {ch && (
 <>
 ...
 <Button size="sm" variant="outline" className="text-xs border-success/50 text-success-muted" onClick={() => setPublishChapter(1)}><Send size={13} /> 从第1章发布</Button>
 </>
 )}
 </>
 )}
 ...
 <div className="flex items-center justify-between mb-2">
 <span className="text-xs font-semibold text-ink"><Lightbulb size={13} className="mr-1" /> AI 大纲建议 — 点击卡片填入生成方向</span>
 ...
 <button className="text-[11px] text-ink-muted hover:text-accent px-2 py-1 rounded border border-border hover:border-accent/30 transition-colors"
 onClick={async () => { const ch = novel?.chapters?.filter((c: {word_count: number}) => c.word_count > 0).pop(); if(ch) { const r=await fetch(`/api/novels/${id}/chapters/${ch.number}/fact-check`); const d=await r.json(); toast.success(`核查完成：${d.issues?.length||0}个疑点`); }}}><Search size={13} className="mr-1" /> 事实核查</button>
 <button className="text-[11px] text-ink-muted hover:text-warn px-2 py-1 rounded border border-border hover:border-amber/30 transition-colors"
 onClick={async () => { const r=await fetch(`/api/novels/${id}/cockpit`); const d=await r.json(); toast.success(`${d.novel}: 均分${d.avg_quality} | ${d.next_actions?.[0]||''}`); }}><BarChart3 size={13} className="mr-1" /> 驾驶舱</button>
 <button className="text-[11px] text-ink-muted hover:text-success px-2 py-1 rounded border border-border hover:border-emerald/30 transition-colors"
 ...
 onClick={async () => { toast.info('正在生成报告…'); const r=await fetch(`/api/novels/${id}/report`); const d=await r.json(); toast.success(`${d.recommendation||''}`); }}><ClipboardList size={13} className="mr-1" /> 出版报告</button>
 ...
 onClick={() => toast.success('EPUB 下载中...')}><FileDown size={13} className="mr-1" /> EPUB</a>
 ...
 }}><Smartphone size={13} className="mr-1" /> MOBI</a>
 <a href={`/api/novels/${id}/export-full`}
 className="text-[11px] text-ink-muted hover:text-accent px-2 py-1 rounded border border-border hover:border-accent/30 transition-colors"
 onClick={() => toast.success('TXT 下载中...')}><FileText size={13} className="mr-1" /> TXT</a>
 <button className="text-[11px] text-ink-muted hover:text-accent px-2 py-1 rounded border border-border hover:border-accent/30 transition-colors"
 onClick={async () => {
 toast.info('正在打包所有章节...');
 const chs = novel.chapters?.filter(c => c.word_count > 0) || [];
 ...
 }}><Lightbulb size={13} className="mr-1" /> 写困救援</button>
 <button
 ...
 >
 <Timer size={13} className="mr-1" /> 时间线
 </button>
 </div>
 ...
 return (
 <div className="mb-3 p-3 rounded-lg bg-warn-soft dark:bg-amber-950/20 border border-warn/20 text-xs animate-[fadeSlideIn_0.3s_ease-out]">
 <span className="text-warn font-medium"><AlertTriangle size={12} className="mr-0.5" /> 质量连续下降</span>
 ...
 {best !== null && (
 <span className="text-ink-muted">
 最佳 <span className="text-success font-semibold">{best.toFixed(2)}</span>
 </span>
 )}
 <span className="text-ink-muted">
 ...
 <h3 className="font-heading text-base font-semibold text-ink mb-3"><Send size={16} className="mr-1.5 text-accent" /> 发布状态</h3>
 <div className="space-y-2 text-[12px]">
 <div className="flex justify-between">
 <span className="text-ink-muted">已发布</span>
 <span className="text-success font-medium">{novel.chapters.filter(c => c.word_count > 0).length} 章</span>
 </div>
 <div className="flex justify-between">
