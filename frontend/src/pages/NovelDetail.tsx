...
export function NovelDetail({ mode }: Props) {
  ...
  function buildGenContext(novelId: string, userDirection: string): string {
    ...
    const allRules = [
      ...
      { rule: '对话只用「说」「问」「道」做标签。不要用「他解释道」「她反驳道」「他冷冷地说」「她惊讶地回应」。对话本身有力量，不需要副词解释。', always: true },
      { rule: '删掉所有「他心想」「她意识到」「他脑子里闪过」。不要报告角色的思想——展示它。如果删不掉，至少减到3处以下。', always: true },
      { rule: '禁止「就在这时」「突然」「此刻」做段落开头。紧张感不靠时间词制造。', always: true },
