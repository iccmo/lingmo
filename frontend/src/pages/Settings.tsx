          ...
          <Card key={p.id} className={`border-border transition-all ${editing === p.id ? 'ring-1 ring-accent/30' : ''}`}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  {(() => {
                    ...
                    return <div className={`w-2 h-2 rounded-full ${tr.ok ? 'bg-emerald-500' : 'bg-red-500'}`} />;
                  })()}
                  ...
                  {(() => {
                    ...
                    if (!tr) return <Badge variant="outline" className="text-[10px] text-amber-500 border-amber-200">检测中...</Badge>;
                    if (tr.ok) return <Badge variant="outline" className="text-[10px] text-emerald-500 border-emerald-200 dark:border-emerald-800">✓ 已连接</Badge>;
                    return <Badge variant="outline" className="text-[10px] text-red-500 border-red-200 dark:border-red-800">✗ 连接失败</Badge>;
                  })()}
                </div>
              ...
              <div className="text-[11px] text-ink-muted space-y-0.5">
                ...
                {/* Test result detail */}
                {testResults[p.id] && (
                  <div className={`text-[11px] ${testResults[p.id].ok ? 'text-emerald-500' : 'text-red-500'}`}>
                    {testResults[p.id].ok
          ...
          <CardContent className="p-4">
            ...
            <div className="flex gap-2 mt-3">
              ...
              <button onClick={() => {
                ...
                className="text-[10px] px-3 py-2 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 dark:border-red-800 transition-colors">
                🗑 清除所有数据
              </button>
            </div>
          </CardContent>
