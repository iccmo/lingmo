  ...
  return (
    <div className="page-enter">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          { v: novels.length, l: '总小说', icon: '📚' },
          { v: totalChapters, l: '总章节', icon: '📖' },
          { v: (totalWords/10000).toFixed(1) + '万', l: '总字数', icon: '📝' },
          { v: avgQualityAll.toFixed(2), l: '均质量', icon: '⭐' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {bestNovel && (
          <Card className="border-emerald-200 dark:border-emerald-800 bg-emerald-50/30 dark:bg-emerald-950/10">
            <CardContent className="p-4">
              <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium uppercase tracking-wide">🏆 质量最高</p>
        ...
        {mostProductive && (
          <Card className="border-accent/30 bg-accent-soft/10">
            <CardContent className="p-4">
              <p className="text-[10px] text-accent font-medium uppercase tracking-wide">⚡ 字数最多</p>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] font-medium ${
                      s.qualityTrend === 'rising' ? 'text-emerald-500' : s.qualityTrend === 'falling' ? 'text-amber-500' : 'text-ink-subtle'
                    }`}>
                      {s.qualityTrend === 'rising' ? '📈 上升' : s.qualityTrend === 'falling' ? '📉 下降' : '➡️ 平稳'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-3 text-[10px]">
                    ...
                    <div>
                      ...
                      <div className={`font-semibold ${s.avgQuality >= 0.7 ? 'text-emerald-500' : s.avgQuality >= 0.55 ? 'text-amber-500' : 'text-red-500'}`}>
