            ...
            <div className="space-y-1">
              {genChapters.slice(-5).map(ch => (
                <div key={ch.number} className="flex items-center gap-2 text-xs">
                  ...
                  {ch.quality_score && (
                    <span className={`tabular-nums font-mono ${ch.quality_score >= 0.7 ? 'text-emerald-500' : ch.quality_score >= 0.5 ? 'text-amber-500' : 'text-red-500'}`}>
            ...
            return (
              <Card className="border-border"><CardContent className="p-4">
                ...
                <div className="text-xs text-ink-muted space-y-1">
                  ...
                  {!fp && <p className="text-ink-subtle">⚠️ 尚未配置灵魂矛盾</p>}
                </div>
              </CardContent></Card>
            );
          })()}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <Card className="border-border"><CardContent className="p-3 text-center">
                  <div className="font-heading text-2xl font-semibold text-amber-500">{foreshadow.total_open}</div>
                  <div className="text-[10px] text-ink-muted">未回收伏笔</div>
                </CardContent></Card>
                <Card className="border-border"><CardContent className="p-3 text-center">
                  <div className="font-heading text-2xl font-semibold text-emerald-500">{foreshadow.total_resolved}</div>
                  <div className="text-[10px] text-ink-muted">已回收伏笔</div>
                </CardContent></Card>
              </div>
              {foreshadow.warning && (
                <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400">
                  ⚠️ {foreshadow.warning}
                </div>
              )}
