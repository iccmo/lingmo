...
def _set_status(novel_id: str, status: str, message: str = "", progress: int = 0):
    with _gen_lock:
...
def _run_generation(novel_id: str):
    ...
    try:
        ...
        def on_stream(text: str):
            ...
            _gen_status[novel_id]["stream_content"] = clean[:3000]
        ...
        while quality['overall'] < q_threshold and retries < max_retries:
            ...
            _set_status(novel_id, "generating", f"质量不足，正在重写…（第{retries+1}次）", 30 + retries * 15)
...
{body}"""
        ...
        while final_quality['overall'] < q_threshold and judge_retries < 2:
            ...
            _set_status(novel_id, "generating", f"LLM 评审未达标，针对性重写…（第{judge_retries}次）", 50 + judge_retries * 15)
