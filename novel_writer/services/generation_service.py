...
class GenerationService:
    ...
    def trigger_generate(self, novel_id: str, req: GenerateRequest) -> GenerateResponse:
        ...
        if current.get("status") in ("generating", "reviewing", "editing", "judging"):
        ...
        ch_count = len(novel.get("chapters", []))
    ...
    def trigger_batch(self, novel_id: str, req: GenerateBatchRequest) -> GenerateResponse:
        ...
        from ..routers.novel._legacy import _run_batch_generation
        import threading
        ...
        thread = threading.Thread(target=_run_batch_generation, args=(novel_id, req.count), daemon=True)
        thread.start()

        log.info(f"Batch generation started: {job_id}", extra={"novel_id": novel_id, "count": req.count})
        ch_count = len(self.db.get_novel(novel_id).get("chapters", []))
        return GenerateResponse(
            job_id=job_id,
            status="queued",
            count=req.count,
