...
_job_queue = _gstate()._job_queue
_agent_memos = _gstate()._agent_memos
...
def _run_queue_job(job: dict):
    ...
    try:
        ...
        # Clean up job entry after 5 minutes to prevent memory leak
        def _cleanup_job():
            import time
            time.sleep(300)
            with _job_lock:
...
def create_novel(data: dict) -> dict:
    ...
    # Auto-create chapter slots so generation doesn't fail on empty novel
    raw_ch = data.get("total_chapters")
    total_ch = int(raw_ch) if raw_ch is not None else 50
    for i in range(1, total_ch + 1):
        db.add_chapter(novel_id=nid, number=i, title=f"第{i}章", word_count=0,
                       content="", summary="", ending_hook="")

    # Re-fetch to get updated chapter count
...
def fix_chapter_formatting(novel_id: str, chapter_num: int) -> dict:
    """Run de_ai on existing chapter to fix orphan quotes and normalize formatting."""
    if not db.get_novel(novel_id):
        raise HTTPException(404, "Novel not found")
    with db.conn() as conn:
        row = conn.execute(
            "SELECT content, title, word_count, summary, ending_hook, quality_score, model_used FROM chapters WHERE novel_id=? AND number=?",
    ...
    gen = Generator.__new__(Generator)  # Skip __init__ — fix_formatting is pure regex, no API key needed
    cleaned, changes = gen.fix_formatting(content)
    db.update_chapter(novel_id, chapter_num, content=cleaned, word_count=len(cleaned))
...
def trigger_generate(novel_id: str, background: BackgroundTasks, data: dict = {}) -> dict:
    ...
    if current_status.get("status") in ("generating", "reviewing", "editing", "judging"):
    ...
    # Pattern disruption — optional, saves 1 LLM call when off
    if (data or {}).get("pattern_disruption"):
        _gen_directions[novel_id + "_pattern_disruption"] = "1"
    _set_status(novel_id, "generating", "正在排队...", 5)
    background.add_task(_run_generation, novel_id)
    ch_count = len([c for c in db.get_novel(novel_id).get("chapters", []) if c.get("word_count", 0) > 0])
...
def novel_farm(data: dict, background: BackgroundTasks) -> dict:
    ...
    for i, seed in enumerate(seeds):
        ...
        from novel_writer.generator import _get_style_for_genre
...
def reorder_chapters(novel_id: str, data: dict) -> dict:
    ...
    # Use a single transaction with temporary negative numbers to avoid UNIQUE conflicts
    with db.conn() as c:
        temp_offset = -100000
        for old_num, new_num in order.items():
            c.execute("UPDATE chapters SET number=? WHERE novel_id=? AND number=?",
                     (int(old_num) + temp_offset, novel_id, int(old_num)))
        for old_num, new_num in order.items():
            c.execute("UPDATE chapters SET number=? WHERE novel_id=? AND number=?",
                     (int(new_num), novel_id, int(old_num) + temp_offset))
    return {"ok": True}
...
def export_epub(novel_id: str) -> dict:
    ...
    # Fetch full content for each chapter (get_novel doesn't include content)
    with db.conn() as conn:
        for ch in gen_chs:
            row = conn.execute("SELECT content FROM chapters WHERE novel_id=? AND number=?", (novel_id, ch["number"])).fetchone()
            ch["content"] = row["content"] if row else ""
...
<p class="synopsis">{novel.get('synopsis','')}</p>"""]
    ...
    safe_fn = quote(f"{novel['title']}_{len(gen_chs)}chapters.html")
...
def export_enhanced_epub_endpoint(novel_id: str):
    """导出增强版EPUB — 含目录+封面+元数据。"""
    from urllib.parse import quote
    html = export_enhanced_epub(db, novel_id)
    if html is None:
        raise HTTPException(404, "Novel not found or no chapters")
    novel = db.get_novel(novel_id)
    fn = f"{novel['title']}_enhanced.epub"
    from fastapi.responses import HTMLResponse
    return HTMLResponse(html, headers={"Content-Disposition": f"attachment; filename*=UTF-8''{quote(fn)}"})
...
def export_enhanced_pdf_endpoint(novel_id: str):
    ...
    if html is None:
        raise HTTPException(404, "Novel not found or no chapters")
    novel = db.get_novel(novel_id)
    fn = f"{novel['title']}_enhanced.pdf"
    from fastapi.responses import HTMLResponse
...
def _run_generation(novel_id: str):
    ...
    next_ch = len([c for c in db.get_novel(novel_id).get("chapters", []) if c.get("word_count", 0) > 0]) + 1
    ...
    try:
        ...
        # Fallback: read soul from database if not in directions
        if not soul_injection:
            try:
                ...
                if fp and fp.get("polarity") and fp.get("answer"):
                    ...
                    p = polarity_map.get(fp["polarity"], (fp["polarity"], ""))
                    soul_injection = f'【灵魂注入 · 核心矛盾：{p[0]}】\n{p[1]}\n\n作者的回答：{fp["answer"]}\n\n写作法则：确保每一章都在追问这个矛盾，但不给出最终答案。'
            except Exception:
                pass
        if soul_injection:
        ...
        while quality['overall'] < q_threshold and retries < max_retries:
            ...
            if retries <= 2 and quality['overall'] >= q_threshold - 0.2:
                ...
                chapter, quality = gen.batch_generate(state, n=1, rag_context=rag_context, outline=outline, style=style, author_input=fix_prompt)
                body = chapter.content or chapter.summary
                quality = gen.score_quality(body, state, style)
                if quality['overall'] >= q_threshold:
                    break
...
{(cleaned_body or body)[:2000]}"""
        ...
        chapter.word_count = len(save_content)
        ...
        try:
            _extract_story_bible(novel_id, chapter.number, save_content, chapter.title)
        ...
        try:
            ...
            _gen_status[novel_id]["quality_trend"] = trend
        except Exception:
            pass
...
def _run_evolve(novel_id: str):
    ...
    from novel_writer.config import Config
    from novel_writer.generator import GENRE_TO_STYLE, STYLE_POOL, WRITER_VOICES, Generator
    provider = _get_provider(novel_id)
    cfg = Config(openai_api_key=provider.get("api_key",""), openai_base_url=provider.get("base_url",""),
                 model=provider.get("models","deepseek-v4-pro")[0] if provider.get("models") else "gpt-4o")
    gen = Generator(cfg)
...
def _run_batch_generation(novel_id: str, count: int):
    ...
    try:
        ...
        if existing_summaries:
            ...
            # Build enriched context as string for the generator
            if isinstance(enriched_context, list):
                enriched_context = summary_text + "\n\n" + "\n".join(
                    str(c) for c in enriched_context if c
                )
            else:
                enriched_context = summary_text + "\n\n" + str(enriched_context or "")
        ...
        for i, outline_item in enumerate(outline[:count]):
            ...
            chapter, quality = gen.batch_generate(state, n=2, rag_context=enriched_context,
            ...
            while quality['overall'] < q_threshold and retries < max_retries:
                retries += 1
                chapter, quality = gen.batch_generate(state, n=1, rag_context=enriched_context,
                                                       outline=outline, style=style)
                body = chapter.content or chapter.summary
...
def _safe_json_list(val):
    """Safely parse JSON string to list, returning [] on any failure."""
    if not val:
        return []
    try:
...
def _run_publish(novel_id: str, chapter_number: int):
    ...
    try:
        ...
        try:
            ...
            # Already in an event loop — use run_coroutine_threadsafe or create task
            import concurrent.futures
            future = asyncio.run_coroutine_threadsafe(pub.publish(novel_id, chapter_number), loop)
...
def _run_expand(novel_id: str, chosen_id: str, direction: str, preview: str, hook: str, edits: str):
    ...
    try:
        from novel_writer.config import Config
        from novel_writer.generator import DraftOption, Generator
        provider = _get_provider(novel_id)
        models = provider.get("models", ["deepseek-v4-pro"])
        model = models[0] if isinstance(models, list) and models else "deepseek-v4-pro"
        cfg = Config(
